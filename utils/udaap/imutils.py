...
def im_to_numpy(img):
    ...
    img = np.ascontiguousarray(img)  # 后加，标记
    return img
...
def resize(img, owidth, oheight):
    ...
    img = skimage.transform.resize(img, tuple(oheight, owidth))
    img = im_to_torch(img)
    print('%f %f' % (img.min(), img.max()))
    return img
...
def sample_with_heatmap(inp, out, num_rows=2, parts_to_show=None):
    ...
    inp_small = skimage.transform.resize(img, (img, tuple(size, size)))
    ...
    for i, part in enumerate(parts_to_show):
        ...
        out_resized = skimage.transform.resize(img, (out[part_idx], tuple(size, size)))
