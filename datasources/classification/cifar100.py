...
class CIFAR100Data:
    def __init__(self):
        ...
        self.inpRes = 32
        self.outRes = 32
        self.train_dataArray, self.valid_dataArray, self.train_labelArray, self.valid_labelArray, self.classes, self.num_classes = self._dataLoading("D:/00Data/cifar100(Classification)/data")
    ...
    def _dataLoading(self, dataPath):
        train_dataArray, train_labelArray, classes, num_classes = self._get_C100Data(dataPath, True)
        valid_dataArray, valid_labelArray, _, _ = self._get_C100Data(dataPath, False)
        return train_dataArray, valid_dataArray, train_labelArray, valid_labelArray, classes, num_classes

    def _get_C100Data(self, dataPath, isTrain):
        dataset = torchvision.datasets.CIFAR100(root=dataPath, train=isTrain, download=True)
